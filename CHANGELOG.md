...
## Version 3.0.3
- **🗂️ Auf-/Abstieg: Liga-Einteilung prüfen & nachjustieren**: Nach „Bestätigte anwenden" (oder per Button „Aktuelle Einteilung anzeigen") erscheint jetzt eine nach Liga gruppierte Übersicht aller gemeldeten Mannschaften der Ziel-Saison. Über ein Dropdown pro Mannschaft lässt sich die Liga direkt ändern (sofort gespeichert). Neu gemeldete Mannschaften ohne Vorjahr stehen übersichtlich unter „Nicht zugewiesen" und können passend eingeordnet werden – so ist die Einteilung an einer Stelle sauber abschließbar. Zusätzlich: Freihand/Pistole (offene Klassen) werden beim Anwenden nicht mehr als vermeintliche Fehler gemeldet, und der Liga-Namensabgleich erkennt jetzt auch ausgeschriebene Varianten („Luftgewehr Freihand" ↔ „LG Freihand")
