...
interface TeamSeasonData extends SeasonData {
  teamName: string;
}
...
interface CrossSeasonComparisonResult {
  seasons: Array<{
    id: string;
    name: string;
    year: number | string;
  }>;
  shooters: Record<string, {
    name: string;
 ...
 * @param numberOfSeasons - Anzahl der zu berücksichtigenden Saisons (Standard: 3)
 * @returns Array mit Leistungsdaten pro Saison
 */
export async function fetchShooterPerformanceHistory(
  shooterId: string, 
  numberOfSeasons = 3
): Promise<SeasonData[]> {
  ...
  try {
    ...
    const scoresBySeason: Record<string, SeasonData> = {};
    ...
    const result = Object.values(scoresBySeason)
      .sort((a, b) => {
        const yearA = typeof a.competitionYear === 'number' ? a.competitionYear : parseInt(a.competitionYear) || 0;
        const yearB = typeof b.competitionYear === 'number' ? b.competitionYear : parseInt(b.competitionYear) || 0;
        return yearB - yearA;
      })
 ...
 * 
 * @param teamId - ID der Mannschaft oder Name der Mannschaft
 * @param clubId - ID des Vereins (optional, wenn teamId ein Name ist)
 * @param numberOfSeasons - Anzahl der zu berücksichtigenden Saisons (Standard: 3)
 * @returns Array mit Leistungsdaten pro Saison
 */
export async function fetchTeamPerformanceHistory(
  teamId: string, 
  clubId?: string, 
  numberOfSeasons = 3
  ...
  try {
    ...
    // Gruppiere Ergebnisse nach Saison
    const scoresBySeason: Record<string, TeamSeasonData> = {};
    const seasonIds = new Set<string>();
    ...
    scoresSnapshot.forEach(doc => {
      ...
      if (!scoresBySeason[seasonId]) {
        scoresBySeason[seasonId] = {
          ...
          shooterName: '', // Wird für Teams nicht verwendet
          scores: [],
          totalScore: 0,
          roundsShot: 0,
          averageScore: 0
 ...
 * @param maxResults - Maximale Anzahl der zurückgegebenen Schützen (Standard: 10)
 * @returns Array mit Schützendaten und Trendanalyse
 */
export async function fetchTopShootersWithTrend(
  ...
  try {
    ...
    const shootersMap = new Map<string, ShooterWithTrend>();
 ...
 * 
 * @param shooterIds - Array mit Schützen-IDs
 * @param numberOfSeasons - Anzahl der zu berücksichtigenden Saisons (Standard: 3)
 * @returns Objekt mit Leistungsdaten pro Schütze und Saison
 */
export async function fetchCrossSeasonComparison(
  shooterIds: string[], 
  numberOfSeasons = 3
  ...
  try {
    ...
    const result: CrossSeasonComparisonResult = {
      seasons,
      shooters: {},
      comparisonData: []
    };
