...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🗑️ Clearing old mannschaften for saison:', saison);
    try {
      ...
      console.log('✅ Cleared', oldDocs.length, 'old mannschaften');
    } catch (error) {
    ...
    try {
      ...
      console.log('✅ Found meldungen for', saison, ':', meldungen.length);
    } catch (error) {
    ...
    console.log('👥 Loading shooters for club mapping...');
    let schuetzen = [];
    try {
      ...
      console.log('✅ Found shooters:', schuetzen.length);
    } catch (error) {
    ...
    console.log('📋 Sample meldung:', meldungen[0]);
    const groups = {};
    ...
    for (const meldung of meldungen) {
      ...
      console.log('🔍 Processing meldung:', { vereinId, disziplinId, schuetzeId: meldung.schuetzeId, schuetzeName: schuetze?.name });
    ...
    for (const [key, group] of Object.entries(groups)) {
      ...
      for (let i = 0; i < sortedMeldungen.length; i += 3) {
        ...
        if (teamMeldungen.length === 3) { // Nur vollständige 3er-Teams, keine Einzelschützen
          ...
          console.log('📊 Team shooters and age groups:', teamSchuetzen.map((s, i) => ({ name: s.name, age: 2026 - s.birthYear, group: wettkampfklassen[i] })));
          ...
          if (uniqueKlassen.length > 1) {
            ...
            if (!isAllowedMix) {
              console.log('⚠️ Skipping invalid mixed team:', uniqueKlassen);
              continue;
            }
          }
    ...
    console.log('✅ Generation complete:', generated, 'teams created');
    return NextResponse.json({
      success: true,
      generated,
